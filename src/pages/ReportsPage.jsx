...
export function ReportsPage() {
  ...
  const report = useMemo(() => {
    const newest = orders.reduce((latest, order) => order.date > latest ? order.date : latest, orders[0]?.date || '2026-08-03');
    ...
    const discounts = paid.reduce((sum, order) => sum + order.discount, 0);
    const refunds = rows.filter((order) => order.paymentStatus === 'Refunded').reduce((sum, order) => sum + order.total, 0);
    const dailyMap = new Map();
    rows.forEach((order) => dailyMap.set(order.date, (dailyMap.get(order.date) || 0) + (order.paymentStatus === 'Paid' ? order.total : 0)));
    ...
    const group = (key) => Object.entries(rows.reduce((result, order) => ({ ...result, [order[key]]: (result[order[key]] || 0) + (order.paymentStatus === 'Paid' ? order.total : 0) }), {})).map(([name, value]) => ({ name, value }));
    const topItems = Object.values(rows.flatMap((order) => order.items).reduce((result, item) => ({ ...result, [item.name]: { name: item.name, quantity: (result[item.name]?.quantity || 0) + item.quantity, sales: (result[item.name]?.sales || 0) + item.price * item.quantity } }), {})).sort((a, b) => b.sales - a.sales).slice(0, 5);
    return { rows, paid, gross, discounts, refunds, net: gross - refunds, daily, byType: group('type'), byPayment: group('paymentMethod'), topItems };
